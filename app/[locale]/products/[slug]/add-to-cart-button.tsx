...
interface Product {
  ...
  price: number;
  stock: number;
  image?: string; // Optional image for the flying animation
}
...
interface AddToCartButtonProps {
  ...
  color?: string; // Custom background color class (e.g., "bg-primary", "bg-red-500")
}
...
export function AddToCartButton({ product, onAddToCart, onStatusChange, color }: AddToCartButtonProps) {
  ...
  const handleClick = async () => {
    ...
    // Simulate Network Request / Cart Logic
    await new Promise((resolve) => setTimeout(resolve, 600));

    // 2. Trigger Success & External Handler
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative">
        ...
        {/* Flying Particles Effect Container */}
        <AnimatePresence>
          {status === "success" && (
            <Particles originRef={buttonRef} />
          )}
        </AnimatePresence>
        ...
        >
          ...
          {/* Background Progress/Fill Animation (Optional Juice) */}
          {status === "loading" && (
            <motion.div
              layoutId="loader-bg"
...
// --- Sub-component: Flying Particles ---
// This creates a burst of particles
function Particles({ originRef }: { originRef: React.RefObject<HTMLButtonElement | null> }) {
...
function Particle({ index, total }: { index: number; total: number }) {
  ...
  const randomDistance = Math.random() * 100 + 50; // 50 to 150px
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        scale: [0, 1, 0], // Scale up then vanish
        opacity: [1, 1, 0],
